...
const FEATURES = [
  ...
  // Market coverage (shared features)
  {
      ...
      <div className="container-xxl">
        ...
        <div className="text-center max-w-4xl mx-auto">
          ...
          <div className="mt-6 inline-block relative sm:pr-8">
            ...
            {billing === "quarterly" && (
              <span className="absolute sm:-right-3 sm:-top-2 top-[110%] left-1/2 -translate-x-1/2 sm:left-auto sm:translate-x-0 inline-flex items-center rounded-full bg-[#3B66F6] text-white text-[11px] font-semibold px-2.5 py-1 shadow-sm ring-1 ring-black/5">
                Optimized pricing
              </span>
            )}
        ...
        <div className="mt-12 grid gap-6 sm:gap-8 md:grid-cols-3">
          <PlanCard title="Free Plan" subtitle="Start exploring with zero cost." price={price.starter} billing={billing} features={FEATURES} planKey="starter" ctaText="Choose Free Plan" />

          <PlanCard title="Unlimited Plan" subtitle="Run high frequency and multiple strategies." price={price.pro} billing={billing} features={FEATURES} planKey="pro" featured ctaText="Choose Unlimited Plan" />

          <PlanCard title="Limited Plan" subtitle="Perfect for testing live deployment with limits." price={price.business} billing={billing} features={FEATURES} planKey="business" ctaText="Choose Limited Plan" />
        </div>
      </div>
    </section>
  );
...
// Single Plan Card
function PlanCard({ title, subtitle, price, billing, features, planKey, ctaText, featured = false }) {
  
  // Filter so each plan shows only its specific feature variant
  const visibleFeatures = features.filter((f) => {
    const keys = ["starter", "pro", "business"];
    const trueCount = keys.filter((k) => f[k]).length;

    if (trueCount > 1) {
      return true; // shared feature appears everywhere
    }

    return f[planKey]; // only show variant for correct plan
  });

  return (
    <div className={[
        "relative rounded-2xl bg-white p-6 sm:p-7 ring-5 transition-all duration-300",
        featured ? "scale-[1.03] ring-[#3B66F6] ring-2 shadow-[0_24px_70px_rgba(59,102,246,0.18)]"
    ...
    >
      ...
      <div className="mt-5 flex items-end gap-1">
        <div className="text-4xl font-extrabold tracking-tight text-neutral-900">₹{price}</div>
      ...
      <button className={[
          ...
          featured ? "text-white bg-gradient-to-r from-[#3B66F6] to-[#1E4CEA] shadow-[0_10px_30px_rgba(40,100,223,0.4)]"
          ...
          return (
            <li key={f.label} className="flex items-start gap-3 text-sm">
              <span className={[
                  ...
                  ok ? "bg-[#3B66F6]/10 text-[#3B66F6] ring-[#3B66F6]/30"
              ...
              <span className={ok ? "text-neutral-800" : "text-neutral-500"}>{f.label}</span>
            </li>
          );
        })}
      </ul>
